...
#filepath = 'output/test0929'
# writer = SummaryWriter(filepath+'/logs')
...
def train(args, device, train_loader, traintest_loader, test_loader):
    ...
    for trial in range(1,args.trials+1):
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            train_epoch(args, model, device, train_loader, optimizer, loss, onoffnoise='off')
            scheduler.step()

            # Compute accuracy on training and testing set
            print("\nSummary of epoch %d:" % (epoch))
            test_epoch(args, model, device, traintest_loader, loss, 'Train',epoch, onoffnoise='off')
            test_epoch(args, model, device, test_loader, loss, 'Test',epoch, onoffnoise='off')
            if args.cont=='True':
